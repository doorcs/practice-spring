...
public class ValidationItemControllerV3 {
    ...
    @InitBinder // 이 어노테이션은 `해당 컨트롤러 클래스`에만 영향을 준다
    ...
    public String addItemV1(
        ...
        // 검증에 실패하면 다시 입력 폼으로 돌아가도록 처리
        if (bindingResult.hasErrors()) {
            log.info("errors = {}", bindingResult);
            // BindingResult는 자동으로 뷰에 같이 넘어가기때문에 modelAttribute 등록은 생략 가능!
    ...
    public String addItemV2(
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1_000_000) {
            bindingResult.addError(new FieldError(
                    "item", "price", item.getPrice(), false, null, null, "가격은 1,000 ~ 1,000,000 까지 허용됩니다."));
        }
        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
            bindingResult.addError(
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10_000) {
                bindingResult.addError(
                        new ObjectError("item", null, null, "가격 * 수량의 합은 10,000 이상이어야 합니다. 현재 값 = " + resultPrice));
            }
        }
